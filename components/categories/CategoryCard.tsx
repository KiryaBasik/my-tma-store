...
interface CategoryCardProps {
  name: string;
  count: number;
  apps: AppPreview[]; // Обновили тип данных
  icon: string;
  colorTheme: string;
  slug: string;
}
...
export default function CategoryCard({
  ...
  const glowColorClass =
    {
      ...
      orange:
        "from-orange-500/20 to-yellow-500/20 group-hover:from-orange-500/30 group-hover:to-yellow-500/30",
  ...
  return (
    <Link href={`/category/${slug}`} className="block h-full">
      ...
      >
        <div
          className={`absolute inset-0 bg-gradient-to-br ${glowColorClass} opacity-0 group-hover:opacity-100 blur-3xl transition-opacity duration-700 pointer-events-none`}
        />
        ...
        <div className="relative z-10 flex justify-between items-start mb-2">
          <div className="flex flex-col gap-4 max-w-[70%]">
            <div
              className={`w-fit p-3 rounded-2xl ${iconBgClass} backdrop-blur-md shadow-sm`}
            >
          ...
          <div className="flex flex-col items-end gap-3">
            <span className="flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-full bg-gray-100 dark:bg-white/5 text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-white/5 group-hover:border-white/20 transition-colors whitespace-nowrap">
              <Sparkles
                size={12}
                className={
                  colorTheme === "blue" ? "text-blue-400" : "text-purple-400"
                }
              />
              {count}
            </span>
        ...
        <div className="relative z-10 flex-grow flex items-center justify-center py-2">
          <div className="relative w-full h-24 flex items-center justify-center">
            {displayApps.map((app, i) => {
              ...
              const firstLetter = app?.title ? app.title[0] : "?";
              ...
              return (
                <motion.div
                  ...
                  className={`absolute w-16 h-16 rounded-[1.2rem] border-[3px] border-white dark:border-[#12141a] bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-800 dark:to-gray-900 shadow-xl ring-1 ${ringColorClass} flex items-center justify-center text-xl font-bold text-gray-500 dark:text-gray-300 overflow-hidden`}
                >
                  {/* Проверяем, есть ли app и icon */}
                  {app && app.icon ? (
            ...
            {/* Кружок "+3" (если есть лишние) */}
            {extraCount > 0 && (
              <motion.div
                variants={{
                  idle: {
                    x:
                      (displayApps.length - (displayApps.length + 1 - 1) / 2) *
                      45,
                    rotate: 15,
                    scale: 0.9,
                    zIndex: 0,
