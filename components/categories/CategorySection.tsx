...
interface SubCategory {
  ...
  apps: string[];
  icon: string;
  // ИСПРАВЛЕНИЕ: Добавили colorTheme, так как она теперь обязательна в карточке
  colorTheme?: string; 
}
...
export default function CategorySection({ id, title, description, icon, color, bgGlow, subcategories }: CategorySectionProps) {
  return (
    <section className="relative w-full">
      <div className={`absolute left-0 top-20 w-full h-[500px] ${bgGlow} blur-[150px] opacity-20 -z-10 pointer-events-none`} />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 px-2">
        {subcategories.map((sub) => (
          <CategoryCard
            ...
            apps={sub.apps}
            icon={sub.icon}
            // ИСПРАВЛЕНИЕ: Передаем colorTheme (если нет, ставим дефолт 'blue')
